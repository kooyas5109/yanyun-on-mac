# Third-party Components

`wine-release` 运行时由以下开源组件构建。

| 组件 | 版本 | 许可证 | 来源 |
| --- | --- | --- | --- |
| Wine | 11.0 | LGPL-2.1 | <https://media.codeweavers.com/pub/crossover/source/crossover-sources-26.1.0.tar.gz> |
| DXMT | 0.80 | LGPL-2.1 | <https://github.com/3Shain/dxmt> |
| DXVK | 3.0.1 | zlib | <https://github.com/doitsujin/dxvk> |
| MoltenVK | 1.4.1 | Apache-2.0 | <https://github.com/KhronosGroup/MoltenVK> |
| GStreamer | 1.28.2 | LGPL-2.1 | <https://gstreamer.freedesktop.org> |
| GLib | 2.88.1 | LGPL-2.1 | <https://gitlab.gnome.org/GNOME/glib> |
| GnuTLS | 3.8.3 | LGPL-2.1 | <https://www.gnutls.org> |
| SDL2 | 2.30.9 | zlib | <https://github.com/libsdl-org/SDL> |
| FreeType | 2.14.3 | FTL | <https://freetype.org> |
| libffi | 3.4.6 | MIT | <https://github.com/libffi/libffi> |
| Wine Gecko | 2.47.4 | MPL-2.0 | <https://wiki.winehq.org/Gecko> |
| Wine Mono | 10.4.1 | MIT | <https://github.com/madewokherd/wine-mono> |

本项目自身的 Swift / C 代码以 [LGPL-2.1](./LICENSE) 发布。

> Portions of this software are copyright © The FreeType Project (<https://freetype.org>).
